...
# LOGGING-----------------------------------
te = open('MicroSD_Readout_log_' + time.strftime("%Y_%m_%d_%Hh_%Mm") + '.txt',
...
# -----------VARIABLES---------------------
target_directory_name = '/Blackvue'
source_directory_name = 'BlackVue/Record'
root_dir = os.getcwd()
msg_temp = ''
flush_delay = 2419200
# -----------------------------------------MAIN-------------------------------------------------------------------------
...
while 1 == 1:
    ...
    if not index:
        ...
        Mbox('STATION-H4',
             'Following SD-Cards were found:\n\n' + msg_temp + '\nIs that correct? (If not hit cancel and make sure all SD-Cards were detected in the File-Explorer and restart program)',
             1)
        ...
        for i in index:
            ...
            if not os.path.isdir(sourcedir):
                Mbox('STATION-H4',
                     'NO SOURCE DIRECTORY FOUND - Make sure SD-Card contains a Record Folder that contains data: ' + sourcedir,
                     1)
            print 'copying to: ' + targetdir
            sync(sourcedir, targetdir, 'sync', verbose=True, modtime=True)
            print '---------------REMOVING OLD DATA in: ' + targetdir + ' --------------- \n'
            flushdir(targetdir, flush_delay)
